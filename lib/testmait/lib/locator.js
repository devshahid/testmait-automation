 ...
 * @type {Array<function(TestMaiT.LocatorOrString, Locator): void>}
 */
Locator.filters = [];

/**
 * Appends new `Locator` filter to an `Locator.filters` array, and returns the new length of the array.
 * @param {function(TestMaiT.LocatorOrString, Locator): void} fn
 * @returns {number}
 */
...
function convertToSubSelector(locator) {
  ...
  if (isXPathStartingWithRoundBrackets(xpath)) {
    throw new Error(
      'XPath with round brackets is not possible here! ' +
