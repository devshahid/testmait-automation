...
function createHelpers(config) {
  ...
  for (const helperName in config) {
    try {
      ...
      if (HelperClass._checkRequirements) {
        ...
        if (requirements) {
          ...
          if (require('./utils').installedLocally()) {
            ...
            console.log(
              'WARNING: TestMaiT is not installed locally. It is recommended to switch to local installation',
            );
      ...
      throw new Error(
        `Could not load helper ${helperName} from module '${moduleName}':\n${err.message}\n${err.stack}`,
      );
    }
  }
...
function createSupportObjects(config) {
  ...
  if (!config.TM) {
    objects.TM = require('./actor')();

    if (container.translation.TM !== 'TM') {
      objects[container.translation.TM] = objects.TM;
    }
  }
...
function createPlugins(config, options = {}) {
  ...
  for (const pluginName in config) {
    ...
    try {
      ...
      throw new Error(
        `Could not load plugin ${pluginName} from module '${module}':\n${err.message}\n${err.stack}`,
      );
    }
  }
  return plugins;
