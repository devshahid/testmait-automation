...
const requireCodeceptConfigure =
  "const { setHeadlessWhen, setCommonPlugins } = require('@codeceptjs/configure');";
const importCodeceptConfigure =
...
const defaultActor = `// in this file you can append custom step methods to 'TM' object

module.exports = function() {
  return actor({

    // Define custom steps here, use 'this' to access default methods of TM.
...
const defaultActorTs = `// in this file you can append custom step methods to 'TM' object
...
module.exports = function (initPath) {
  ...
  print();
  print(`  Welcome to ${colors.magenta.bold('TestMaiT')} initialization tool`);
  print('  It will prepare and configure a test environment for you');
  print();
  print(' Useful links:');
  print();
  print('  👉 How to start testing ASAP: https://testmait.io/quickstart/#init');
  print('  👉 How to select helper: https://testmait.io/basics/#architecture');
  print('  👉 TypeScript setup: https://testmait.io/typescript/#getting-started');
  print();
  ...
  const configFile = path.join(testsPath, 'testmait.conf.js');
  if (fileExists(configFile)) {
    error(`Config is already created at ${configFile}`);
    return;
  }

  const typeScriptconfigFile = path.join(testsPath, 'testmait.conf.ts');
  ...
  inquirer
    .prompt([
      ...
      },
      {
        ...
        default: true,
        message:
          'Do you want to use JSONResponse helper for assertions on JSON responses? http://bit.ly/3ASVPy9',
    ...
    .then((result) => {
      ...
      const finish = async () => {
        ...
        fs.writeFileSync(
          path.join(testsPath, stepFile),
          extension === 'ts' ? defaultActorTs : defaultActor,
        );
        ...
        if (isTypeScript) {
          configSource = beautify(
            `export const config : TestMaiT.MainConfig = ${inspect(config, false, 4, false)}`,
          );
          ...
          configSource = beautify(
            `/** @type {TestMaiT.MainConfig} */\nexports.config = ${inspect(
              config,
              false,
              4,
              false,
        ...
        const generateDefinitionsManually = colors.bold(
          `To get auto-completion support, please generate type definitions: ${colors.green(
            'npx codeceptjs def',
          )}`,
        );
        ...
        if (packages) {
          try {
            ...
            print(colors.bold.magenta('Find more information at https://testmait.io'));
            return;
          }
        }
        ...
        generatedTest.then(() => {
          print('\n--');
          print(colors.bold.green('TestMaiT Installed! Enjoy supercharged testing! 🤩'));
          print(colors.bold.magenta('Find more information at https://testmait.io'));
          print();
        });
      };
...
function install(dependencies) {
  ...
  if (!fs.existsSync(path.join(process.cwd(), 'package.json'))) {
    ...
    throw new Error(
      "Error: 'package.json' file not found. Generate it with 'npm init -y' command.",
    );
  }

  if (!installedLocally()) {
    console.log('TestMaiT should be installed locally');
    dependencies.push('codeceptjs');
  }
...
function _actorTranslation(stepFile, translationSelected) {
  ...
  for (const translationAvailable of translations) {
    ...
    if (translationSelected === translationAvailable) {
      const nameOfActor = require('../../translations')[translationAvailable].TM;
