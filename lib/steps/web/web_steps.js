...
Then(/^I grab the total number of cars category for special edition$/, async () => {
  const cars = await HotWheelPage.getCategoryText();
  console.log('Total Categories for special edition: ', cars);
  TestData.setField('special_cars', cars);
});

Then(/^I download hotwheels data with below instructions$/, async (table) => {
  const specialCars = TestData.getField('special_cars');
  let tObj = table.parse().hashes()[0];

  if (specialCars && specialCars.length > 0) {
    let isFromUpdate = true;
    let isToUpdate = true;
    for (let mainCarsCount = 0; mainCarsCount < specialCars.length; mainCarsCount++) {
      let { cars: mainCarsList } = await HotWheelPage.getNumberOfCars(
        mainCarsCount + 1,
        'updatedCarList',
      );
      let mainCount = 0;
      console.log('mainCarsList: ', mainCarsList);

      if (tObj.mainfrom && tObj.mainfrom !== 'all' && isFromUpdate) {
        mainCount = Number(tObj.mainfrom) - 1;
        isFromUpdate = false;
      }

      if (tObj.mainTo && tObj.mainTo !== 'all' && isToUpdate) {
        if (tObj.mainTo.includes('-')) {
          const [min, max] = tObj.mainTo.split('-');
          mainCount = Number(min);
          mainCarsList = Number(max);
        } else {
          mainCarsList = Number(tObj.mainTo);
        }
        isToUpdate = false;
      }

      for (let carCount = mainCount; carCount < mainCarsList; carCount++) {
        const rootLocator = CommonUtils.identifyLocator('updatedCarList')
          .replace('REPLACE_ID', mainCarsCount + 1)
          .replace('REPLACE_SUB_ID', carCount + 1);

        console.log('rootLocator: ', rootLocator);

        // TM.executeScript(() => {
        //   const footer = document.querySelector('.top-ads-container');
        //   const footer1 = document.querySelector('.wds-tabs__tab-label');
        //   footer1.style.display = 'none';
        //   footer.style.display = 'none';
        // });

        await TM.waitForElement(`${rootLocator}//a`, 15);
        // await TM.scrollIntoView(locator);
        await TM.scrollTo(CommonUtils.identifyLocator(rootLocator), 0, -100);

        await TM.waitForElement(`${rootLocator}//a`, 15);
        const currentCategory = await TM.grabTextFrom(`${rootLocator}//a`);

        console.log(
          'Selected Category : ',
          currentCategory,
          `(${carCount + 1}) out of ${mainCount}`,
        );
        console.log('Current folder category: ', specialCars[mainCarsCount]);
        await TM.wait(2);
        console.log('Click Locator: ', `${rootLocator}//a`);
        await TM.click(`${rootLocator}//a`);
        await TM.wait(2);
        // TM.executeScript(() => {
        //   const footer = document.querySelector('.top-ads-container');
        ...
        // Store the web URL to new:
        const url = await TM.grabCurrentUrl();
        TestData.setField('webURL', url);
        await mainScrapFn(table);

        if (CommonUtils.identifyData('mainWebURL').includes('hotwheels.fandom.com')) {
          // / Redirect back to the URL
          await TM.amOnPage(CommonUtils.identifyData('mainWebURL'));
          console.log('\n\n');
        }
      }
    }
...
async function mainScrapFn(table) {
  let tObj = table.parse().hashes()[0];
  ...
  const otherYears = [
    ...
    'Ultra Hot Series',
    'Larry Garage 2009',
    'Larry Garage Holidays',
    'Hot wheels garage 2010',
    'Hot wheels garage 2011',
    ...
    'Color_Changers',
    'Chopcycles',
    'Vintage_Racing_Series_(2011)',
    'The_Hot_Ones_(2011)',
    'Team_Hot_Wheels_High-Speed_Wheel_Series',
    'Super_Chromes_Series_(2006)',
    'Speed_Machines_(2010)',
    'Hot_Wheels_Racing_Series_(2012)',
    'Hot_Wheels_Heritage',
    // 'Highway_35_World_Race',
    'High_Speed_Racing_Wheels_Series',
    'Garage_Series',
  ];

  const specialYears = ['1996', '2005', '2004', '2002', '2001', 'Flip Outs'];

  let categoryCount = 0;
  if (tObj.categoryName !== 'all') {
    if (tObj.categoryName && !isNaN(tObj.categoryName)) {
      // its a number
      categoryCount = Number(tObj.categoryName);
    } else {
      const count = categories.findIndex(
        (category) => category.trim() === tObj.categoryName.trim(),
      );
  ...
  if (tObj.categoryLoop !== 'all') {
    if (tObj.categoryLoop && !isNaN(tObj.categoryLoop)) {
      // its a number
      len = Number(tObj.categoryLoop) + 1;
    } else {
      const count = categories.findIndex(
        (category) => category.trim() === tObj.categoryName.trim(),
      );
  ...
  for (let category = categoryCount; category < len; category++) {
    ...
    for (let i = count; i < cars; i++) {
      ...
      // else if (mixTableYears.includes(tObj.year)) {
      // }
      else {
        locator = CommonUtils.identifyLocator(tObj.locator ?? 'carList')
          .replace('REPLACE_ID', category + 1)
          .replace('REPLACE_SUB_ID', i + 1);
      }
      ...
      if (years.includes(tObj.year)) {
        ...
        locator = CommonUtils.identifyLocator('specialCarList')
          .replace('REPLACE_ID', category + 1)
          .replace('REPLACE_SUB_ID', i + 1);
      }

      console.log('locator: ', locator);
      const ads = await TM.grabNumberOfVisibleElements(
        CommonUtils.identifyLocator('ads-container'),
      );
      if (ads > 0) {
        TM.executeScript(() => {
          const elements = [
      ...
      const horizontalScroll = await TM.grabNumberOfVisibleElements('.fl-scrolls');
      if (horizontalScroll > 0) {
        TM.executeScript(() => {
          const element = document.querySelector('.fl-scrolls');
          if (element) element.style.display = 'none';
        });
      }
      ...
      const ads1 = await TM.grabNumberOfVisibleElements(
        CommonUtils.identifyLocator('ads-container'),
      );
      if (ads1 > 0) {
        TM.executeScript(() => {
      ...
      // const subcategories = await TM.grabNumberOfVisibleElements(
      //   CommonUtils.identifyLocator(
      //     'carList'.replace('[REPLACE_ID]', '').replace('[REPLACE_SUB_ID]', ''),
      //   ),
      // );
      // console.log('Subcategories Lenght: ', subcategories);
      // if (subcategories === 0) {
      const url = await TM.grabCurrentUrl();
      ...
      // }
      // else {
      //   console.log('Inside subcategories condition');
      //   let value = await TM.grabCurrentUrl();
      //   TestData.setField('subcategories_urls', value);
      //   for (const sub of Array.from({ length: subcategories }, (_, index) => index)) {
      //     const locator1 = CommonUtils.identifyLocator('subcategories');
      //     // TM.executeScript(() => {
      //     //   const footer = document.querySelector('.top-ads-container');
      //     //   const footer1 = document.querySelector('.wds-tabs__tab-label');
      //     //   footer1.style.display = 'none';
      ...
      //     await TM.waitForElement(`${locator1}[${sub + 1}]`, 15);
      //     // await TM.scrollIntoView(locator);
      //     await TM.scrollTo(`${locator1}[${sub + 1}]`, 0, -100);

      //     await TM.waitForElement(`${locator1}[${sub + 1}]`, 15);
      //     const currentCategory = await TM.grabTextFrom(`${locator1}[${sub + 1}]`);

      //     console.log(
      //       'Selected Category : ',
      //       currentCategory,
      //       `(${sub + 1}) out of ${subcategories}`,
      //     );
      //     console.log('Current folder category: ', categories[category]);
      //     await TM.wait(2);
      //     console.log('Click Locator: ', `${locator1}[${sub + 1}]`);
      //     await TM.click(`${locator1}[${sub + 1}]`);
      //     await TM.wait(2);
      ...
      //       console.log('Category already exists (partial match)');
      //       // Redirect back to the URL
      //       await TM.amOnPage(CommonUtils.identifyData('subcategories_urls'));
      //       console.log('\n\n');
      //       continue;
      //     }
...
async function commonScrapFn(
  ...
  if (tObj.listAndCategoryNameCheck === 'true' && carData.category === currentCategory) {
    ...
    if (isLocatorExist > 0) {
      ...
      tableHeadersList = await TM.grabTextFromAll(CommonUtils.identifyLocator('tableHeadersList'));
    } else {
    ...
    tableHeadersList = tableHeadersList.filter(
      (header) =>
        (header.toLowerCase().includes('version') || header.toLowerCase().includes('releases')) &&
    ...
    const categoryFolderPath = path.join(folderPath, categories[category].replace(/[^\w]/g, '_'));
    ...
    if (currentCategory !== carData.category) {
      fileName = path.join(
        categoryFolderPath,
        `${currentCategory.replace(/^[^\w]/, '').replace(/[^\w]/g, '_')}_${carData.category.replace(
          /[^\w]/g,
          '_',
        )}.json`,
      );
