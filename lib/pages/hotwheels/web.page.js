...
class HotWheelPage {
  ...
  async getAllLiTexts(locator = 'newCategoryList') {
    ...
    if (allTexts.length > 0) {
      return allTexts.filter(
        (text) => text != 'Description' && text !== 'Gallery' && text != 'See Also',
      );
    }
  ...
  async getCategoryText(categoryName) {
    ...
    const categoryText = await TM.grabTextFromAll(
      '//table/preceding-sibling::h3|//table/preceding-sibling::h4',
    );
    // return ['', 'Real Riders Series', 'Redline Series'];
  ...
  async getNumberOfCars(category, locator, specialYears, otherYears, year) {
    ...
    if (specialYears && specialYears.includes(year)) {
      ele = CommonUtils.identifyLocator('specialCarList')
