...
const { event, output } = require('../../../testmait');
 ...
 */
  ...
  plugin.addCommonMetadata = () => {
    ...
    plugin.addLabel('framework', 'testmait');
  };
  ...
  function finishMetastep(level) {
    ...
    metaStepsToFinish.forEach(() => {
      ...
      if (
        reporter.suites &&
        reporter.suites.length &&
        reporter.suites[0].currentStep &&
        reporter.suites[0].currentStep.constructor.name === 'Step'
      ) {
        reporter.endStep('passed');
      }
    });
