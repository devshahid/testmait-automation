...
declare namespace TestMaiT {
  ...
  ${methodsTypeFragment}
  interface TM extends ${helpersListFragment} {}
...
module.exports = function (genPath, options) {
  ...
  for (const name in helpers) {
    const require = testmait.config.helpers[name].require;
    if (require) {
      helperPaths[name] = require;
      helperNames.push(name);
    } else {
      const fullBasedPromised = testmait.config.fullPromiseBased;
  ...
  supportObject.set('TM', 'TM');
  supportObject.set('current', 'any');

  if (translations.loaded) {
    supportObject.set(translations.TM, translations.TM);
  }
  ...
  for (const name in testmait.config.include) {
    const includePath = testmait.config.include[name];
    if (name === 'TM' || name === translations.TM) {
  ...
  if (translations.loaded) {
    ...
    namespaceTranslationAliases.push(
      `interface ${translations.vocabulary.TM} extends WithTranslation<Methods> {}`,
    );
    ...
    if (translations.vocabulary.contexts) {
      Object.keys(translations.vocabulary.contexts).forEach((k) => {
        translationAliases.push(
          `declare const ${translations.vocabulary.contexts[k]}: typeof ${k};`,
        );
      });
    }
...
function getPath(originalPath, targetFolderPath, testsPath) {
  ...
  if (parsedPath.base.endsWith('.d.ts'))
    parsedPath.base = parsedPath.base.substring(0, parsedPath.base.length - 5);
