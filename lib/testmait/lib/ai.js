...
const debug = require('debug')('testmait:ai');
...
class AiAssistant {
  ...
  setHtmlContext(html) {
    ...
    if (this.htmlConfig.maxLength)
      processedHTML = splitByChunks(processedHTML, this.htmlConfig.maxLength)[0];
  ...
  async createCompletion(messages) {
    ...
    debug(messages);
    output.debug('Inside CreateCompletion');

    if (this.mockedResponse) return this.mockedResponse;

    this.response = null;
    output.debug('Create Chat Completion Start');
    try {
      const completion = await this.openai.createChatCompletion({
        ...this.config,
        messages,
      });
      output.debug('Create Chat Completion Done');
      this.response = completion?.data?.choices[0]?.message?.content;

      debug(this.response);
      output.debug(`Got respnase back ${this.response}`);
  ...
  async healFailedStep(step, err, test) {
    ...
    const messages = [
      ...
      {
        role: 'user',
        content: `I want to heal a test that fails. Here is the list of executed steps: ${test.steps.join(
          ', ',
        )}`,
      },
      ...
      {
        role: 'user',
        content: `Here is HTML code of a page where the failure has happened: \n\n${this.html}`,
      },
    ];
    output.debug(`Sending Message to OpenAI ${messages}`);
    const response = await this.createCompletion(messages);
    output.debug(`OpenAI Responded back`);
    if (!response) return [];
    ...
    const messages = [
      {
        ...
        content: `I am test engineer writing test in CodeceptJS
          I have opened web page and I want to use CodeceptJS to ${input} and use only css locator on this page
      ...
      },
      {
        role: 'user',
        content:
          'Propose only CodeceptJS steps code. Do not include Scenario or Feature into response',
      },
...
function parseCodeBlocks(response) {
  ...
  const modifiedSnippets = codeSnippets.map((snippet) => {
    ...
    const filteredLines = lines
      .map((line) => {
        // Replace occurrences of "I." with "TM."
        return line.replace(/I\./g, 'TM.');
      })
      .filter(
        (line) =>
          !line.includes('TM.amOnPage') &&
  ...
  }); // .filter(snippet => !line.includes('TM.amOnPage'));
