...
const Testmait = require('../lib/testmait');
...
program.version(Testmait.version());
...
program
  .command('list [path]')
  .alias('l')
  .description('List all actions for TM.')
  .action(errorHandler(require('../lib/command/list')));

program
  .command('def [path]')
  .description('Generates TypeScript definitions for all TM actions.')
  .option('-c, --config [file]', 'configuration file to be used')
...
program
  ...
  .alias('bdd:init')
  .description('Prepare TestMaiT to run feature files.')
  .option('-c, --config [file]', 'configuration file to be used')
