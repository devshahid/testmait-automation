  ...
  return inquirer
    ...
    .then((result) => {
      ...
      if (container.translation().loaded) {
        ...
        testContent = testContent.replace('{{actor}}', container.translation().TM);
        if (vocabulary.contexts.Feature)
          testContent = testContent.replace('Feature', vocabulary.contexts.Feature);
        if (vocabulary.contexts.Scenario)
          testContent = testContent.replace('Scenario', vocabulary.contexts.Scenario);
        output.print(
          `Test was created in ${colors.bold(
            config.translation,
          )} localization. See: https://testmait.io/translation/`,
        );
      } else {
        testContent = testContent.replace('{{actor}}', 'TM');
      }
...
const pageObjectTemplate = `const { TM } = inject();
...
const poModuleTemplateTS = `const { TM } = inject();
...
const poClassTemplate = `const { TM } = inject();
...
module.exports.pageObject = function (genPath, opts) {
  ...
  let configFile = path.join(testsPath, `testmait.conf.${extension}`);

  if (!fileExists(configFile)) {
    extension = 'ts';
    configFile = path.join(testsPath, `testmait.conf.${extension}`);
  }
  ...
  return inquirer
    ...
    .then((result) => {
      ...
      if (config.include.TM) {
        let actorPath = config.include.TM;
        if (actorPath.charAt(0) === '.') {
          // relative path
          actorPath =
            path.relative(dir, path.dirname(path.join(testsPath, actorPath))) +
            actorPath.substring(1); // get an upper level
        }
      ...
      output.print(`Your config file (${colors.cyan(
        'include',
      )} section) has included the new created PO:
    ...
    ...
      ...
      output.print(
        `Use ${output.colors.bold(
          colors.cyan(name),
        )} as parameter in test scenarios to access this object:`,
      );
      output.print(`\nScenario('my new test', ({ TM, ${name} })) { /** ... */ }\n`);
      ...
      try {
        ...
        output.print(
          `Run ${colors.green(
            'npx codeceptjs def',
          )} to update your types to get auto-completion for object.`,
        );
      }
    });
};

const helperTemplate = `const Helper = require('testmait/helper');
