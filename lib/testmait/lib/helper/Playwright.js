...
class Playwright extends Helper {
  ...
  async _startBrowser() {
    if (this.isElectron) {
      ...
      try {
        this.browser = await playwright[this.options.browser].connectOverCDP(
          this.playwrightOptions,
        );
      } catch (err) {
   ...
   *
   * @param {TestMaiT.LocatorOrString} locator element located by CSS|XPath|strict locator.
  ...
  async dragAndDrop(srcElement, destElement, options) {
    ...
    await this.page.mouse.move(
      destElBox.x + destElBox.width / 2,
      destElBox.y + destElBox.height / 2,
    );
  ...
  async clickLink(locator, context = null) {
    ...
    console.log('Replace TM.clickLink with TM.click');
   ...
   *
   * @param {TestMaiT.LocatorOrString} locator field located by label|name|CSS|XPath|strict locator.
  ...
  async seeNumberOfElements(locator, num) {
    ...
    return equals(
      `expected number of elements (${new Locator(locator)}) is ${num}, but found ${
        elements.length
      }`,
  ...
  async seeCssPropertiesOnElements(locator, cssProperties) {
    ...
    return equals(
      `all elements (${new Locator(locator)}) to have CSS property ${JSON.stringify(
        cssProperties,
      )}`,
  ...
  async seeAttributesOnElements(locator, attributes) {
    ...
    return equals(
      `all elements (${new Locator(locator)}) to have attributes ${JSON.stringify(attributes)}`,
    ).assert(chunked.length, elemAmount);
  }
  ...
  async _passed(test) {
    ...
    if (this.options.trace) {
      if (this.options.keepTraceForPassedTests) {
        if (this.options.trace) {
          test.artifacts.trace = await saveTraceForContext(
            this.browserContext,
            `${test.title}.passed`,
          );
          for (const sessionName in this.sessionPages) {
  ...
  async waitForEnabled(locator, sec) {
    ...
    if (!locator.isXPath()) {
      const valueFn = function ([locator]) {
        return (
          Array.from(document.querySelectorAll(locator)).filter((el) => !el.disabled).length > 0
        );
      };
  ...
  async waitForValue(field, value, sec) {
    ...
    if (!locator.isXPath()) {
      ...
      const valueFn = function ([locator, $XPath, value]) {
        eval($XPath); // eslint-disable-line no-eval
        return (
          $XPath(null, locator).filter((el) => (el.value || '').indexOf(value) !== -1).length > 0
        );
      };
  ...
  async waitForElement(locator, sec) {
    ...
    try {
      await context
        ...
        .first()
        .waitFor({ timeout: waitTimeout, state: 'attached' });
    } catch (e) {
      throw new Error(
  ...
  async grabRecordedNetworkTraffics() {
    if (!this.recording || !this.recordedAtLeastOnce) {
      throw new Error(
        'Failure in test automation. You use "I.grabRecordedNetworkTraffics", but "I.startRecordingTraffic" was never called before.',
      );
    }
  ...
  async seeTraffic({ name, url, parameters, requestPostData, timeout = 10 }) {
    if (!name) {
      throw new Error('Missing required key "name" in object given to "TM.seeTraffic".');
    }

    if (!url) {
      throw new Error('Missing required key "url" in object given to "TM.seeTraffic".');
    }

    if (!this.recording || !this.recordedAtLeastOnce) {
      throw new Error(
        'Failure in test automation. You use "TM.seeTraffic", but "TM.startRecordingTraffic" was never called before.',
      );
    }
    ...
    if (requestPostData && this._isInTraffic(url)) {
      ...
      assert.equal(
        advancedTestResults,
        true,
        `Traffic named "${name}" found correct URL ${url}, BUT the post data did not match:\n ${advancedTestResults}`,
      );
  ...
  grabTrafficUrl(urlMatch) {
    ...
    assert.fail(
      `Method "getTrafficUrl" failed: No request found in traffic that matches ${urlMatch}`,
    );
  }
  ...
  dontSeeTraffic({ name, url }) {
    if (!this.recordedAtLeastOnce) {
      throw new Error(
        'Failure in test automation. You use "I.dontSeeTraffic", but "I.startRecordingTraffic" was never called before.',
      );
    }
    ...
    if (this._isInTraffic(url)) {
      assert.fail(
        `Traffic with name "${name}" (URL: "${url}') found, but was not expected to be found.`,
      );
    }
  }
  ...
  _isInTraffic(url, parameters) {
    ...
    this.requests.forEach((request) => {
      ...
      if (parameters) {
        const advancedReport = allParameterValuePairsMatchExtreme(
          extractQueryObjects(request.url),
          parameters,
        );
  ...
  _getWebSocketLog(prefix, payload) {
    return `${prefix} ID: ${payload.requestId} TIMESTAMP: ${
      payload.timestamp
...
async function targetCreatedHandler(page) {
  ...
  page.on('console', (msg) => {
    if (
      !consoleLogStore.includes(msg) &&
      this.options.ignoreLog &&
      !this.options.ignoreLog.includes(msg.type())
    ) {
      this.debugSection(
...
async function saveVideoForPage(page, name) {
  ...
  const fileName = `${`${
    global.output_dir
  }${pathSeparator}videos${pathSeparator}${uuidv4()}_${clearString(name)}`.slice(0, 245)}.webm`;
  page
    .video()
    .saveAs(fileName)
    .then(() => {
...
async function saveTraceForContext(context, name) {
  ...
  const fileName = `${`${
    global.output_dir
  }${pathSeparator}trace${pathSeparator}${uuidv4()}_${clearString(name)}`.slice(0, 245)}.zip`;
...
const createAdvancedTestResults = (url, dataToCheck, requests) => {
  ...
  requests.forEach((request) => {
    ...
    if (!request.requestPostData) {
      advancedResults = allParameterValuePairsMatchExtreme(
        extractQueryObjects(request.url),
        dataToCheck,
      );
    } else if (request.requestPostData) {
      advancedResults = allRequestPostDataValuePairsMatchExtreme(
...
const allParameterValuePairsMatchExtreme = (
  queryStringObject,
  advancedExpectedParameterValuePairs,
) => {
      ...
      if (queryParameter.name === expectedKey) {
        ...
        if (expectedValue === undefined) {
          ...
          if (decodedActualValue === expectedValue.base64) {
            littleReport += `   ${expectedKey.padStart(10, ' ')} = base64(${
              expectedValue.base64
            })\n`;
          } else {
            littleReport += ` ✖ ${expectedKey.padStart(10, ' ')} = base64(${
              expectedValue.base64
            })     -> actual value: "base64(${decodedActualValue})"\n`;
            success = false;
          }
        } else if (queryParameter.value === expectedValue) {
          littleReport += `   ${expectedKey.padStart(10, ' ')} = ${expectedValue}\n`;
        } else {
          littleReport += ` ✖ ${expectedKey.padStart(
            10,
            ' ',
          )} = ${expectedValue}      -> actual value: "${queryParameter.value}"\n`;
          success = false;
        }
      }
    }
...
const allRequestPostDataValuePairsMatchExtreme = (
  RequestPostDataObject,
  advancedExpectedRequestPostValuePairs,
) => {
