...
const defaultUser = {
  fetch: (TM) => TM.grabCookie(),
  check: () => {},
  restore: (TM, cookies) => {
    TM.amOnPage('/'); // open a page
    TM.setCookie(cookies);
  },
};
 ...
 *       },
 *       check: (I, session) => {
 *          // Throwing an error in `check` will make TestMaiT perform the login step for the user
...
module.exports = function (config) {
  ...
  const loginFunction = async (name) => {
    ...
    const shouldAwait =
      isAsyncFunction(userSession.login) ||
      isAsyncFunction(userSession.restore) ||
      isAsyncFunction(userSession.check);

    const loginAndSave = async () => {
      if (shouldAwait) {
        await userSession.login(TM);
      } else {
        userSession.login(TM);
      }

      const cookies = await userSession.fetch(TM);
      if (!cookies) {
      ...
      if (config.saveToFile) {
        ...
        fs.writeFileSync(
          path.join(global.output_dir, `${name}_session.json`),
    ...
    if (shouldAwait) {
      await userSession.restore(TM, cookies);
      await userSession.check(TM, cookies);
    } else {
      userSession.restore(TM, cookies);
      userSession.check(TM, cookies);
    }
