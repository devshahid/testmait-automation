...
/**
 * Testmait-style interface:
 *
 * Feature('login');
 *
 * Scenario('login as regular user', ({TM}) {
 *   TM.fillField();
 *   TM.click();
 *   TM.see('Hello, '+data.login);
 * });
 *
...
module.exports = function (suite) {
  ...
  suite.on('pre-require', (context, file, mocha) => {
    ...
    context.Feature = function (title, opts) {
      ...
      suite.beforeEach('testmait.before', () => scenario.setup(suite));
      afterEachHooks.push(['finalize testmait', () => scenario.teardown(suite)]);

      suite.beforeAll('testmait.beforeSuite', () => scenario.suiteSetup(suite));
      afterAllHooks.push(['testmait.afterSuite', () => scenario.suiteTeardown(suite)]);
