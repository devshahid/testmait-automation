...
Then(/^I download hotwheels data with below instructions$/, async (table) => {
  ...
  for (let category = categoryCount; category < len; category++) {
    let cars = await HotWheelPage.getNumberOfCars(category + 1, 'carList');
    if (cars === 0) {
      cars = await HotWheelPage.getNumberOfCars(category, 'carRoot');
    }
    console.log('Total Car Category: ', cars);
    ...
    for (let i = count; i < cars; i++) {
      ...
      let locator = CommonUtils.identifyLocator('carList')
      ...
      await TM.scrollTo(CommonUtils.identifyLocator(locator), 0, -500);

      await TM.waitForElement(locator, 15);
      const currentCategory = await TM.grabTextFrom(locator);
      console.log('Selected Category : ', currentCategory);
      await TM.wait(2);
      await TM.click(locator);
      await TM.wait(2);
      const ele = await TM.grabNumberOfVisibleElements(locator);
      if (ele > 0) {
        await TM.click(locator);
      }
