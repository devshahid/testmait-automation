...
class JSONResponse extends Helper {
  ...
  _beforeSuite() {
    ...
    if (!this.helpers[this.options.requestHelper]) {
      throw new Error(
        `Error setting JSONResponse, helper ${
          this.options.requestHelper
        } is not enabled in config, helpers: ${Object.keys(this.helpers)}`,
      );
    }
  ...
  seeResponseContainsJson(json = {}) {
    ...
    if (Array.isArray(this.response.data)) {
      ...
      assert.ok(
        fails < this.response.data.length,
        `No elements in array matched ${JSON.stringify(json)}`,
      );
    } else {
  ...
  seeResponseMatchesJsonSchema(fnOrSchema) {
    ...
    if (!joi.isSchema(schema))
      throw new Error('Invalid Joi schema provided, see https://joi.dev/ for details');
