...
const { getConfig, getTestRoot, updateConfig, safeFileWrite } = require('../utils');
...
const stepsFile = `const { TM } = inject();
...
module.exports = function (genPath) {
  ...
  output.print('Initializing Gherkin (Cucumber BDD) for TestMaiT');
  output.print('--------------------------');

  if (config.gherkin && config.gherkin.steps) {
    output.error(
      'Gherkin is already initialized in this project. See `gherkin` section in the config',
    );
    process.exit(1);
  }
