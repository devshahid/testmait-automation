...
const parser = _interopDefault(require('parse-function'))({
  parse: acorn.parse,
  ecmaVersion: 11,
...
function getParams(fn) {
  ...
  try {
    ...
    if (reflected.args.length > 1 || reflected.args[0] === 'TM') {
      error('Error: old TestMaiT v2 format detected. Upgrade your project to the new format -> ');
    }
