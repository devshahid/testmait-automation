...
const configFileNames = ['testmait.conf.js'];
...
class Config {
  ...
  static load(configFile) {
    ...
    if (!fileExists(configFile)) {
      ...
      if (!fileExists(configFile)) {
        throw new Error(
          `Config file ${configFile} does not exist. Execute 'testmait init' to create config`,
        );
      }
    }
    ...
    throw new Error(
      `Can not load config from ${configPaths}\ntestmait is not initialized in this dir. Execute 'testmait init' to start`,
    );
  }
...
function loadConfigFile(configFile) {
  ...
  if (extensionName === '.ts') {
    try {
      ...
      console.log('ts-node package is required to parse testmait.conf.ts config correctly');
    }
  }
