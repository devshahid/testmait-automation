 ...
 * @prop {boolean|number} [smartWait=false] - **enables [SmartWait](http://testmait.io/acceptance/#smartwait)**; wait for additional milliseconds for element to appear. Enable for 5 secs: "smartWait": 5000.
...
class WebDriver extends Helper {
  ...
  _validateConfig(config) {
    ...
    if (!config.capabilities.platformName && (!config.url || !config.browser)) {
      throw new Error(`
        ...
        Check your testmait config file to ensure these are set properly
          {
            "helpers": {
              "WebDriver": {
                "url": "YOUR_HOST"
  ...
  _isCustomLocator(locator) {
    ...
    if (locatorObj.isCustom()) {
      ...
      throw new Error(
        'Please define "customLocatorStrategies" as an Object and the Locator Strategy as a "function".',
      );
    }
    return false;
  }
  ...
  async _locateShadow(locator) {
    ...
    for (let index = 0; index < shadow.length; index++) {
      ...
      if (!elements || !elements[0]) {
        throw new Error(
          `Shadow Element '${shadowElement}' is not found. It is possible the element is incorrect or elements sequence is incorrect. Please verify the sequence '${shadowSequence.join(
            '>',
  ...
  async _locate(locator, smartWait = false) {
    ...
    if (locator.react) {
      const els = await this.browser.react$$(
        locator.react,
        locator.props || undefined,
  ...
  async click(locator, context = null) {
    const clickMethod =
      this.browser.isMobile && !this.browser.isW3C ? 'touchClick' : 'elementClick';
    const locateFn = prepareLocateFn.call(this, context);
    ...
    if (context) {
      assertElementExists(
        ...
        locator,
        'Clickable element',
        `was not found inside element ${new Locator(context)}`,
      );
    } else {
  ...
  async grabCssPropertyFromAll(locator, cssProperty) {
    const res = await this._locate(locator, true);
    const val = await forEachAsync(res, async (el) =>
      this.browser.getElementCSSValue(getElementId(el), cssProperty),
    );
  ...
  async seeCssPropertiesOnElements(locator, cssProperties) {
    ...
    return assert.ok(
      ...
      `expected all elements (${new Locator(locator)}) to have CSS property ${JSON.stringify(
        cssProperties,
      )}`,
    );
  }
  ...
  async seeAttributesOnElements(locator, attributes) {
    ...
    return assert.ok(
      chunked.length === elemAmount,
      `expected all elements (${new Locator(locator)}) to have attributes ${JSON.stringify(
        attributes,
      )}`,
    );
  }
  ...
  async scrollTo(locator, offsetX = 0, offsetY = 0) {
    ...
    if (locator) {
      ...
      const elem = usingFirstElement(res);
      const elementId = getElementId(elem);
      if (this.browser.isMobile && !this.browser.isW3C)
        return this.browser.touchScroll(offsetX, offsetY, elementId);
    ...
    if (this.browser.isMobile && !this.browser.isW3C)
      return this.browser.touchScroll(locator, offsetX, offsetY);
  ...
  async saveScreenshot(fileName, fullPage = false) {
    ...
    if (this.activeSessionName) {
      ...
      if (browser) {
        this.debug(
          `Screenshot of ${this.activeSessionName} session has been saved to ${outputFile}`,
        );
    ...
    return this.browser.waitUntil(
      async () => {
        ...
        const selected = await forEachAsync(res, async (el) =>
          this.browser.isElementEnabled(getElementId(el)),
        );
  ...
  async waitForText(text, sec = null, context = null) {
    ...
    return this.browser.waitUntil(
      async () => {
        ...
        const selected = await forEachAsync(res, async (el) =>
          this.browser.getElementText(getElementId(el)),
        );
        if (Array.isArray(selected)) {
  ...
  async waitNumberOfVisibleElements(locator, num, sec = null) {
    ...
    return this.browser.waitUntil(
      ...
      {
        timeout: aSec * 1000,
        timeoutMsg: `The number of elements (${new Locator(
          locator,
        )}) is not ${num} after ${aSec} sec`,
      },
    );
  }
  ...
  async waitForInvisible(locator, sec = null) {
    ...
    return this.browser.waitUntil(
      ...
      {
        timeout: aSec * 1000,
        timeoutMsg: `element (${new Locator(locator)}) still visible after ${aSec} sec`,
      },
    );
  }
  ...
  async waitForDetached(locator, sec = null) {
    ...
    return this.browser.waitUntil(
      ...
      {
        timeout: aSec * 1000,
        timeoutMsg: `element (${new Locator(locator)}) still on page after ${aSec} sec`,
      },
    );
  }
  ...
  async switchToNextTab(num = 1, sec = null) {
    ...
    await this.browser.waitUntil(
      ...
      {
        timeout: aSec * 1000,
        timeoutMsg: `There is no ability to switch to next tab with offset ${num}`,
      },
    );
  ...
  async switchToPreviousTab(num = 1, sec = null) {
    ...
    await this.browser.waitUntil(
      ...
      {
        timeout: aSec * 1000,
        timeoutMsg: `There is no ability to switch to previous tab with offset ${num}`,
      },
    );
    return this.browser.switchToWindow(target);
  }
...
async function proceedSeeField(assertType, field, value) {
  ...
  const proceedMultiple = async (fields) => {
    const fieldResults = toArray(
      await forEachAsync(fields, async (el) => {
        const elementId = getElementId(el);
        return this.browser.isW3C
          ? el.getValue()
          : this.browser.getElementAttribute(elementId, 'value');
      }),
    );
  ...
  const proceedSingle = (el) =>
    this.browser.getElementAttribute(getElementId(el), 'value').then((res) => {
      if (res === null) {
...
async function proceedSeeCheckbox(assertType, field) {
  ...
  const selected = await forEachAsync(res, async (el) =>
    this.browser.isElementSelected(getElementId(el)),
  );
