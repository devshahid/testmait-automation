...
Then(/^I download hotwheels data "([^"]*)"$/, async (year) => {
  ...
  for (let i = 1; i < cars; i++) {
    ...
    if (tableHeaders === 0 || tableHeaders <= 0) {
      tableHeaders = await TM.grabNumberOfVisibleElements(
        CommonUtils.identifyLocator('AllTableHeadersDifferentLocator'),
      );
    }
  ...
  fs.writeFileSync(`${year}-cars.json`, JSON.stringify(carsJSON, null, 2), 'utf-8');
});
